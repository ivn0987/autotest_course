...
def josephus_task(num_people, kill_num):
    """
    Функция принимает два целых числа num_people и kill_num и возвращает последний элемент списка
    :param num_people: int Число людей
    :param kill_num: int Номер выбывающего
    :return: (survivor) int Номер "выжившего"
    """
    people_list = []
    surv_people = 0
    for people in range(1, num_people + 1):
        surv_people = (surv_people + kill_num) % people
    survivor = surv_people + 1
    return survivor
