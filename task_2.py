...
class PersonInfo:
    ...
    def __init__(self, name, age, *subdivisions):
        """
        Конструктор принимает ФИО, возраст и иерархию подразделений
        :param name: str ФИО сотрудника
        :param age: int Возраст сотрудника
        :param args: [str] Иерархия подразделений сотруника
        """
        self.name = name
        self.age = age
    ...
    def short_name(self):
        """
        Метод возвращает короткое ФИО сотрудника в формате "Фамилия И."
        :return: str ФИО в формате "Фамилия И."
        """
        name_arr = self.name.split(' ')
        return f'{name_arr[1]} {name_arr[0][0]}.'
    ...
    def new_salary(self):
        """
        Метод возвращает новую з/п по формуле
        1337*Возраст*суммарное кол-во вхождений трех наиболее часто встречающихся букв из списка подразделений
        :return: int Новая зарплата
        """
        sum_count = 0
        join_sub = ''.join(self.subdivisions)
        max_count = Counter(join_sub).most_common(3)
        for count in max_count:
            sum_count += count[1]
        return 1337 * self.age * sum_count
