...
def repeats(our_str):
    """
    Фунция принимает строку our_str и возвращает новую строку с подставленным числом
    уже имеющихся вхождений после каждого символа в формате _n
    :param our_str: str Строка
    :return: (new_str) Строка с добавленным постфиксом формата _n
    """
    new_str = ''
    our_dict = {item: 0 for item in our_str}
    our_list = list(our_str)
    for item in our_list:
        our_dict[item] = our_dict[item] + 1
        new_str = f'{new_str}{item}_{our_dict[item]}'
    return new_str
